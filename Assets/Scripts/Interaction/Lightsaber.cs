...
public class Lightsaber : MonoBehaviour {
    ...
    private void updateSwingHandler()
    {
        ...
        if (saberActive) // TODO: consider scale, i. e bladeScaleCurrent == bladeScaleMax)
        {
            foreach (Blade blade in blades)
            {
                if (swingSpeed > 1) // TODO: just random swing values; needs to me more generic
                {
    ...
    private void ToggleLightsaberOnOff(string sword)
    {
        if (saberActive)
        {
            LightsaberOff(sword);
    ...
    private void LightsaberOn(string sword)
    {
        foreach (Blade blade in blades)
        {
    ...
    private void LightsaberOff(string sword)
    {
        foreach (Blade blade in blades)
        {
            if(blade.gameObject.name == sword)
